...
public class LoginInterceptor implements HandlerInterceptor {
	//1.preHandler : 요청이 컨트롤러의 함수(Handler)에 도달하기 전에 작동한다
	//2.postHandler : 컨트롤러의 함수(Handler)가 실행된 이후 view(jsp)로 넘어가기 전에 작동한다
	//3.afterCompletion : view(jsp)가 싱행된 이후 작동한다
	@Override
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		// header.jsp를 사용할 수도 있지만, 모든 페이지에 영향을 주기 때문에 일부페이지에만 적용하기 쉽지않다
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
			throws Exception {
		// 사용자에게 JSP의 화면이 모두 전달 되고, 응답이 완료된 후 작동한다
